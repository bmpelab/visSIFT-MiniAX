...
void CFastCamMonitorDlg::OnTimer(UINT nIDEvent)
{
	...
	case TIMER_MODE_PLAY:
			...
			{
				if(m_nRecFrame <= 0){
					msg = _T("There is no frame for playback");
				...
				}else if( m_nEndFrame < m_PlayFrameCount)
				{
                    ...
                    m_disp_status1.SetWindowText(_T("Playback has cmopleted"));

                    msg = _T("Playback has cmopleted");
			...
			if(m_Execute_Record==TRUE){
				...
				if(nStatus != PDC_STATUS_REC && nStatus != PDC_STATUS_RECREADY){
					...
					nRet = PDC_SetStatus(nDeviceNo,
										 ...
										 &nErrorCode);
					if (nRet == PDC_FAILED) {
                        msg = _T("error PDC_SetStatus");
						AfxMessageBox(msg,MB_OK|MB_ICONWARNING);
						return;
					}

                    m_disp_status1.SetWindowText(_T("Recording has completed"));

                    msg = _T("Recording has completed");
					AfxMessageBox(msg,MB_OK|MB_ICONINFORMATION);
					m_PlayFrameCount = 0;
...
void CFastCamMonitorDlg::OnShowWindow(BOOL bShow, UINT nStatus)
{
    ...
    m_disp_status1.SetWindowText(_T("Stop"));

	// initialize PDCLIB
	nRet = PDC_Init(&nErrorCode);
	if (nRet == PDC_FAILED) {
        msg = _T("eroor PDC_Init");
		AfxMessageBox(msg,MB_OK|MB_ICONWARNING);
		return;
	}
	// device search
	...
	nRet = PDC_GetExistChildDeviceList(nDeviceNo,
									   ...
									   &nErrorCode);
	if (nRet == PDC_FAILED) {
        msg = _T("error PDC_GetExistChildDeviceList");
		AfxMessageBox(msg,MB_OK|MB_ICONWARNING);
		return;
	}
	...
	nRet = PDC_GetDeviceName(nDeviceNo,
							...
							&nErrorCode);
	if(nRet == PDC_FAILED) {
		msg = _T("error PDC_GetDeviceName");
		AfxMessageBox(msg,MB_OK|MB_ICONWARNING);
		return;
	}
	...
	nRet = PDC_GetRecordRateList(nDeviceNo,
							...
							&nErrorCode);
	if(nRet == PDC_FAILED) {
        msg = _T("error PDC_GetRecordRateList");
		AfxMessageBox(msg,MB_OK|MB_ICONWARNING);
		return;
	}
	...
	nRet = PDC_SetRecordRate(nDeviceNo,
							...
							&nErrorCode);
	if(nRet == PDC_FAILED) {
		msg = _T("error PDC_SetRecordRate");
		AfxMessageBox(msg,MB_OK|MB_ICONWARNING);
		return;
	}
...
void CFastCamMonitorDlg::DspLiveCamera()
{
	...
	nRet = PDC_GetLiveImageData(nDeviceNo,
                            ...
                            &nErrorCode);
	if (nRet == PDC_FAILED) {
        msg = _T("error PDC_GetLiveImageData");
		AfxMessageBox(msg,MB_OK|MB_ICONWARNING);	
		delete [] m_BmpImage;
		return;
	}
...
void CFastCamMonitorDlg::DspPlayCamera()
{
	...
	nRet = PDC_GetMemImageData(nDeviceNo,			
							   ...
							   &nErrorCode);
	if (nRet == PDC_FAILED) {
        msg = _T("error PDC_GetMemImageData");
		AfxMessageBox(msg,MB_OK|MB_ICONWARNING);	
		delete [] m_BmpImage;
		return;
	}
...
void CFastCamMonitorDlg::OnCbnSelchangeChangeRate()
{
	...
	if(m_Execute_Live==FALSE){
        msg = _T("Please change to Live Mode");
		AfxMessageBox(msg,MB_OK|MB_ICONWARNING);	
		return;
	}
...
void CFastCamMonitorDlg::OnCbnSelchangeChangeShutter()
{
	...
	nRet = PDC_SetShutterSpeedFps(nDeviceNo,
									...
									&nErrorCode);
	if(nRet == PDC_FAILED){
        msg = _T("error PDC_SetShutterSpeedFps");
		AfxMessageBox(msg,MB_OK|MB_ICONWARNING);	
		EndWaitCursor();
		return;
	}
...
void CFastCamMonitorDlg::OnCbnSelchangeChangeResolution()
{
	...
	nRet = PDC_SetResolution(nDeviceNo,
							...
							&nErrorCode);
	if(nRet == PDC_FAILED){
        msg = _T("error PDC_SetResolution");
		AfxMessageBox(msg,MB_OK|MB_ICONWARNING);	
		EndWaitCursor();	
		return;
	}
...
void CFastCamMonitorDlg::OnBnClickedModeLive()
{
	...
	nRet = PDC_GetResolution(nDeviceNo,
                     ...
                     &nErrorCode);
	if (nRet == PDC_FAILED) {
        msg = _T("error PDC_GetResolution");
		AfxMessageBox(msg,MB_OK|MB_ICONWARNING);	
	}
    ...
    m_disp_status1.SetWindowText(_T("Live"));
	m_Execute_Live = TRUE;
...
void CFastCamMonitorDlg::OnBnClickedModeMemory()
{
	...
	nRet = PDC_GetMemFrameInfo(nDeviceNo,
								...
								&nErrorCode);
	if(nRet == PDC_FAILED) {
        msg = _T("error PDC_GetMemFrameInfo");
		AfxMessageBox(msg,MB_OK|MB_ICONWARNING);
		return;
	}
    ...
    m_disp_status1.SetWindowText(_T("Memory"));
	...
	nRet = PDC_GetMemResolution(nDeviceNo,
                     ...
                     &nErrorCode);
	if (nRet == PDC_FAILED) {
        msg = _T("error PDC_GetMemResolution");
		AfxMessageBox(msg,MB_OK|MB_ICONWARNING);	
	}
	DspPlayCamera();
...
void CFastCamMonitorDlg::OnBnClickedRecordMemory()
{
    ...
    m_disp_status1.SetWindowText(_T("Rec ready"));
    ...
    m_disp_status1.SetWindowText(_T("Recording"));
...
void CFastCamMonitorDlg::OnBnClickedPlayMemory()
{
	...
	BeginWaitCursor();
    m_disp_status1.SetWindowText(_T("Play"));
...
void CFastCamMonitorDlg::OnBnClickedRecordSaveHd()
{
    ...
    m_disp_status1.SetWindowText(_T("Download"));

    CFileDialog myDLG(FALSE, NULL, _T("FastCam"), OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, _T("bitmap(*.bmp)|*.bmp|all(*.*)|*.*||"));
	if(myDLG.DoModal() == IDOK)
	{
		...
		_tcscat_s(cPath,cTime);			
		_tcscpy_s(cPathTemp,cPath);			
        ...
        m_disp_status1.SetWindowText(_T("Download End"));
...
void CFastCamMonitorDlg::OnBnClickedProgramExit()
{
    ...
    msg = _T("Do you want to exit program?");
...
void CFastCamMonitorDlg::UpdateParam() 
{
	...
	nRet = PDC_GetShutterSpeedFpsList(nDeviceNo,
										...
										&nErrorCode);
	if(nRet == PDC_FAILED){
        msg = _T("error PDC_GetShutterSpeedFpsList");
		AfxMessageBox(msg,MB_OK|MB_ICONWARNING);	
		EndWaitCursor();	
		return;
	}
	...
	nRet = PDC_GetResolutionList(nDeviceNo,
										...
										&nErrorCode);
	if(nRet == PDC_FAILED){
        msg = _T("error PDC_GetResolutionList");
		AfxMessageBox(msg,MB_OK|MB_ICONWARNING);	
		EndWaitCursor();
		return;
	}	
	...
	nRet = PDC_SetTriggerMode(nDeviceNo,
								...
								&nErrorCode);
	if(nRet == PDC_FAILED){
        msg = _T("error PDC_SetTriggerMode");
		AfxMessageBox(msg,MB_OK|MB_ICONWARNING);	
		EndWaitCursor();
		return;
	}
