...
void CFastCamMonitorDlg::DspPlayCamera()
{
	...
	sprintf_s(cWk0,"FrameNo.[%d]",m_PlayFrameCount);
...
void CFastCamMonitorDlg::OnBnClickedRecordSaveHd()
{
	...
	if(myDLG.DoModal() == IDOK)
	{
		...
		strcpy_s(cPath,myDLG.GetPathName());
		sprintf_s(cTime,"%4.4d%2.2d%2.2d_%2.2d%2.2d%2.2d\\",tm.wYear,tm.wMonth,tm.wDay,tm.wHour,tm.wMinute,tm.wSecond);
		strcat_s(cPath,cTime);			//タイムスタンプを追加
		strcpy_s(cPathTemp,cPath);			//フォルダのディレクトリを一時保存
		...
		for(i=m_nStartFrame; i<=m_nEndFrame; i++){
			...
			//	BMPファイルを作成
			//-----------------------------
			strcpy_s(cPath,cPathTemp);
			sprintf_s(cWk0,"Temp%4.4d.bmp",i);
			strcat_s(cPath,cWk0);
			//カラー・モノクロ情報の取得
			nRet = PDC_GetColorType(nDeviceNo,
						 nChildNo,
...
void CFastCamMonitorDlg::UpdateParam() 
{
	...
	for(i=0; i<nSize; i++){
			sprintf_s(cWk0,"1/%d",nShutterList[i]);
	...
	for(i=0; i<nSize; i++){
		...
		sprintf_s(cWk0,"%dx%d",nWidth,nHeight);
