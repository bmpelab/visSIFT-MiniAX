	...
	case TIMER_MODE_PLAY:
			if(!m_IsDraw)
			{
				...
				}else if( m_nEndFrame < m_PlayFrameCount)
				{
					...
					m_disp_status1.SetWindowText(_T("再生終了"));
	...
	case TIMER_MODE_RECORD:			//録画(メモリ)への録画終了チェック
			...
			if(m_Execute_Record==TRUE){
				...
				if(nStatus != PDC_STATUS_REC && nStatus != PDC_STATUS_RECREADY){
					...
					m_disp_status1.SetWindowText(_T("録画終了"));
...
void CFastCamMonitorDlg::OnShowWindow(BOOL bShow, UINT nStatus)
{
    ...
    m_disp_status1.SetWindowText(_T("モニタ停止"));
	//--------------------
	//指定カメラの初期化処理
	//--------------------
	...
	nRet = PDC_GetExistChildDeviceList(nDeviceNo,
									   ...
									   &nErrorCode);
	if (nRet == PDC_FAILED) {
        msg = _T("接続されている子デバイス数を取得できませんでした");
		AfxMessageBox(msg,MB_OK|MB_ICONWARNING);
		return;
	}
	// 子デバイスナンバーをリストに表示 
...
void CFastCamMonitorDlg::OnCbnSelchangeChangeRate()
{
	...
	if(m_Execute_Live==FALSE){
        msg = _T("ライブ画像表示に切り替えてから実行してください");
		AfxMessageBox(msg,MB_OK|MB_ICONWARNING);	
		return;
	}
...
void CFastCamMonitorDlg::OnBnClickedModeLive()
{
    ...
    m_disp_status1.SetWindowText(_T("ライブ画像表示中"));
...
void CFastCamMonitorDlg::OnBnClickedModeMemory()
{
    ...
    m_disp_status1.SetWindowText(_T("メモリ"));
	//-----------------------
	//	画像表示領域をクリア
	//-----------------------
	ClearMovieRangeWindow(); 
...
void CFastCamMonitorDlg::OnBnClickedRecordMemory()
{
    ...
    m_disp_status1.SetWindowText(_T("録画準備中"));
    ...
    m_disp_status1.SetWindowText(_T("録画中"));
...
void CFastCamMonitorDlg::OnBnClickedPlayMemory()
{
    ...
    m_disp_status1.SetWindowText(_T("再生中"));
...
void CFastCamMonitorDlg::OnBnClickedRecordSaveHd()
{
    ...
    m_disp_status1.SetWindowText(_T("ファイル保存中"));

    CFileDialog myDLG(FALSE, NULL, _T("FastCam"), OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, _T("ビットマップ(*.bmp)|*.bmp|全て(*.*)|*.*||"));
	if(myDLG.DoModal() == IDOK)
	{
		...
		_tcscat_s(cPath,cTime);			    //タイムスタンプを追加
        _tcscat_s(cPathTemp, cPath);		//フォルダのディレクトリを一時保存
        ...
        m_disp_status1.SetWindowText(_T("ファイル保存終了"));
...
void CFastCamMonitorDlg::UpdateParam() 
{
	...
	nRet = PDC_GetShutterSpeedFpsList(nDeviceNo,
										...
										&nErrorCode);
	if(nRet == PDC_FAILED){
        msg = _T("シャッタースピードリストが取得できませんでした");
		AfxMessageBox(msg,MB_OK|MB_ICONWARNING);	
		EndWaitCursor();	// 砂時計カーソルを消去します。
		return;
	}
